...
import businessInsuranceData from '../../newsdata/businessNewsData.json'; // path to your business insurance data JSON
import Footer from '../Footer';
import Navbar from '../Navbar';

const BusinessInsuranceNews = () => {
  const [expandedIndex, setExpandedIndex] = useState(null);

  if (!businessInsuranceData || businessInsuranceData.length === 0) {
  ...
  return (
    <div>
      ...
      <div className="news-card-container">
        {businessInsuranceData.map((article, index) => {
          const businessInfo = article.BusinessInsuranceInformation;
              ...
              <div className="news-card-content">
                ...
                {isExpanded && (
                  <div className="news-expanded-content">
                    {sectionKeys.map((key) => {
                      const heading = businessInfo[`heading${key}`];
                      const content = businessInfo[`content${key}`];
                      return (
...
export default BusinessInsuranceNews;
