...
import healthArticleData from '../../newsdata/healthNewsData.json'; // path to your health news JSON
...
const HealthNewsCard = () => {
  ...
  if (!healthArticleData || healthArticleData.length === 0) {
  ...
  return (
    <div>
      ...
      <div className="news-card-container">
        {healthArticleData.map((article, index) => {
          const healthInfo = article.HealthInsuranceInformation;
              ...
              <div className="news-card-content">
                ...
                {isExpanded && (
                  <div className="news-expanded-content">
                    {sectionKeys.map((key) => {
                      const heading = healthInfo[`heading${key}`];
                      const content = healthInfo[`content${key}`];
                      return (
