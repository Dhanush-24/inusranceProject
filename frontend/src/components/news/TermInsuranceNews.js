...
import termInsuranceData from '../../newsdata/termNewsData.json'; // path to your term insurance data JSON
...
const TermInsuranceNews = () => {
  ...
  if (!termInsuranceData || termInsuranceData.length === 0) {
  ...
  return (
    <div>
      ...
      <div className="news-card-container">
        {termInsuranceData.map((article, index) => {
          const termInfo = article.TermInsuranceInformation;
              ...
              <div className="news-card-content">
                ...
                {isExpanded && (
                  <div className="news-expanded-content">
                    {sectionKeys.map((key) => {
                      const heading = termInfo[`heading${key}`];
                      const content = termInfo[`content${key}`];
                      return (
